...
def deal_data(data: dict) -> tuple:
    ...
    first_name_combine = get_repeat(get_low_and_up_str([first_name])) if re.match(
        '^[a-zA-Z0-9]+$', first_name) else ['']
    last_name = [data.get('second_name', ''), data.get('third_name', '')]
    last_name_combine = get_repeat(get_low_and_up_str([''.join(last_name)])) if re.match(
        '^[a-zA-Z0-9]+$', ''.join(last_name)) else ['']
    ...
    # birthday
    birthday, birthday2, birthday_all = data.get(
    ...
    for i in [telephone, mobile]:
        if re.match('^[0-9-]+$', i):
            phone_all += (i.split('-') +
                          [''.join(i.split('-')), i]) if i.find('-') > -1 else [i]
            # 没有去掉-号，暂时不考虑这种情况
            phone_all += get_repeat(get_head_tail(i, 3, 4, 5, 6))
    # other，其他
    user_name = data.get('user_name', '')
    account = data.get('account', '')
    qq = data.get('qq', '') if re.match(
        '^[0-9]{5,13}$', data.get('qq', '')) else ''
    organization = data.get('organization', '') if re.match(
        '^[0-9a-zA-Z@./ -]+$', data.get('organization', '')) else ''
    company = data.get('company', '') if re.match(
        '^[0-9a-zA-Z@./ -]+$', data.get('company', '')) else ''
    like_use = data.get('like_use', '')
    other = data.get('other', '')
    other_all = [user_name, account, qq,
                 organization, company, like_use, other]
    other_all += [j for i in other_all for j in get_repeat(
        get_head_tail(i, 3, 4))]
    # id_card，身份证号
    id_card = data.get('id_card', '') if re.match(
        r'^\d{17}(x|X|\d)$', data.get('id_card', '')) else ''
    id_card_all = [] if len(id_card) != 18 else distinct_list(get_repeat(
        get_head_tail(id_card, 3, 4, 6, 8) + get_head_tail(id_card[:-1], 3, 4, 6, 8)))
    # work_no，工号
    work_no = data.get('work_no', '') if re.match(
        '^[0-9a-zA-Z@./ -]+$', data.get('work_no', '')) else ''
...
def index_get():
    ...
    filter_list = ["number_filter", "string_filter",
                   "short_filter", "long_filter", "short", "long"]
    number_filter, string_filter, short_filter, long_filter, short, long = [
        data.get(i, '') for i in filter_list]
    # 开始生成二阶和三阶密码
    pass_second, pass_third = [], []
    for i, j in enumerate(pass_list_all):
        if connector:
            pass_third += [''.join(m)
                           for m in itertools.product(j, connector, j)]
        for k in pass_list_all[:i] + pass_list_all[i+1:]:
            pass_second += [''.join(m) for m in itertools.product(j, k)]
            if connector:
                pass_third += [''.join(m)
                               for m in itertools.product(j, connector, k)]
    pass_second += [''.join(m) for m in itertools.product(
        pass_list_all[6], pass_list_all[6])]  # other特殊处理
    ...
    pass_list = [drop_short_long(distinct_list(
        get_capitalize(i)), start=short, end=long) for i in pass_list]
    if number_filter:
