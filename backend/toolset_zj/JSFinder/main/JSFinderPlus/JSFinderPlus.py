...
from toolset.JSFinder.main.JSFinderPlus.core.FindThread import UrlProcuder, UrlConsumer, InfoOutput, ScanThread
...
from toolset.JSFinder.main.JSFinderPlus.module.argparse import Arg
from toolset.JSFinder.main.JSFinderPlus.module.banner import banner
from toolset.JSFinder.main.JSFinderPlus.module.color import color
from toolset.JSFinder.main.JSFinderPlus.module.proxy import proxy_set
...
def run(url):
    ...
    with open(globals.get_value("ROOT_PATH") + "/toolset/JSFinder/main/JSFinderPlus/module/dict.txt", "r", encoding='utf-8') as f:
        line = f.readline()
        while line:
    ...
    for t in t_list:
        ...
        t.join()
        if t.name.startswith("信息输出线程"):
            result = t.get_result()
            print(color.green(f"输出结果：{result}"))
    return result


if __name__ == "__main__":
    run(url="http://103.45.116.194/")
