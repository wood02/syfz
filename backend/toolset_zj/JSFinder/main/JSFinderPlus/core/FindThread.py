...
from toolset.JSFinder.main.JSFinderPlus.core.core import Core
from toolset.JSFinder.main.JSFinderPlus.module import globals
from toolset.JSFinder.main.JSFinderPlus.module.color import color
        ...
        while True:
            try:
                if self.scan_queue.empty():
                    break
                url = self.scan_queue.get(block=True, timeout=5)
                html, code = Core.Extract_html(url)
                if code != 404:
...
class InfoOutput(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                type, url, code, text = self.output_queue.get(block=True, timeout=8)
                if type == 1:
                    if code == 200:
        ...
        if globals.get_value("SUBDOMIAN_LIST"):
            ...
            for line in globals.get_value("SUBDOMIAN_LIST"):
                ...
                format_data = {"id": str(id), "url": line, "info": "subdomain",
                               "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}
                # tables = tables + (
                #     table.format(id=str(id), url=line, info="subdomain",
                #                  time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                tables = tables + (table.format(**format_data))
                syfzOutList.append(format_data)
                id = id + 1
        ...
        for url, code, l in riskList:
            ...
            #                                 time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
            format_data = {"id": str(id), "url": url, "info": "code:{} -- size:{}".format(str(code), str(l)),
                           "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}
            tables = tables + (table.format(**format_data))
            syfzOutList.append(format_data)
            id = id + 1
        ...
        for k, m, url in globals.get_value("leak_infos_match"):
            ...
            format_data = {"id": str(id), "url": url, "info": "{} / {}".format(str(k), str(m)),
                           "time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}
            tables = tables + (table.format(**format_data))
            syfzOutList.append(format_data)
            id += 1
        ...
        # html = ""
        # with open(globals.get_value("ROOT_PATH") + "/toolset/JSFinder/main/JSFinderPlus/module/report.html", "r", encoding='utf-8') as f:
        #     html = f.read()
        #
        ...
        print(color.green("溯源反制所需数据：") + json.dumps(syfzOutList, ensure_ascii=False))

        self.result = syfzOutList
